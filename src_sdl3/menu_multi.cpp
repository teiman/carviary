...
// m_net_cursor: 0 = IPX, 1 = TCP/IP. Serial/direct removed.
...
// 0 = IPX, 1 = TCP/IP. Serial/modem entries removed.
...
/* SERIAL/MODEM config menus removed: this port has no serial-line networking */
...
static void ExtraMaps_ScanDir(const char *dirpath)
{
	char mapname[64];
	int count;
	char **files = SDL_GlobDirectory(dirpath, "*.bsp", SDL_GLOB_CASEINSENSITIVE, &count);
	if (!files)
		return;

	for (int i = 0; i < count && extramaps_count < 127; i++)
	{
		COM_StripExtension(files[i], mapname);
...
void ExtraMaps_Init (void)
{
	...
	{
		char dirpath[128];
		snprintf(dirpath, sizeof(dirpath), "%s/maps", moddir);
		ExtraMaps_ScanDir(dirpath);
	}
}