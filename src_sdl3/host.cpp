...
qboolean	offline_mode = false;	// bypass network stack in local single player
...
double	sv_progs_time = 0; // ms spent in SV_Physics (progs) per frame
...
void Host_Error (char *error, ...)
{
	...
	cl.intermission = 0;			// clear intermission to avoid stuck screen
...
// always pause in single player if in console or menus
	if (!sv.paused && (svs.maxclients > 1 || key_dest == key_game) )
	{
		...
		sv_progs_time = (double)(t2 - t1) / (double)freq * 1000.0;
	}
...
// update offline mode flag
	offline_mode = (sv.active && svs.maxclients == 1 &&
