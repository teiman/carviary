...
Returns true if the file exists in the search path (including PAK files)
===========
*/
qboolean COM_FileExists (char *filename)
{
	FILE	*f = NULL;
	int		ret;

	ret = COM_FindFile (filename, NULL, &f);
	if (f)
		fclose (f);
	return ret != -1;
}
...
//
	...
	// Load all .pak files from the directory using SDL3
	int count = 0;
	char **files = SDL_GlobDirectory(dir, "*.pak", SDL_GLOB_CASEINSENSITIVE, &count);
	if (files)
	{
		for (i = 0; i < count; i++)
		{
			if (files[i][0] == '.')
				continue;
			_snprintf(pakfile, sizeof(pakfile), "%s/%s", dir, files[i]);
