	...
	//	Con_Printf("%s\n", texname);
		...
		else
		{
			if (!strncmp(tx->name,"sky",3) && mt->width == 256 && mt->height == 128)
			{
				tx->width	= mt->width;
				tx->height	= mt->height;
				R_InitSky ((byte *)((intptr_t) mt + mt->offsets[0]), 1);
			}
			else
			{
				if (mt->offsets[0])
				{
					data				= (byte *)((intptr_t) mt + mt->offsets[0]);
					if (data)
					{
						tx->width			= mt->width;
						tx->height			= mt->height;
						...
						if (fullbrights)
						{
							data2 = (byte *)malloc (tx->width * tx->height);
				...
				else
				{
					...
					tx->gl_texturenum	= GL_LoadTexture ("notexture", tx->width, tx->height, (byte *)((intptr_t) r_notexture_mip + r_notexture_mip->offsets[0]), true, false, 1);
				}
			}
		}
	}
...
void Mod_LoadHLLighting (lump_t *l)
{
	...
	loadmodel->lightdata = (byte *)Hunk_AllocName ( l->filelen, va("%s lightmaps", loadname));
