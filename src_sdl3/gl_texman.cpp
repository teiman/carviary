...
void R_ResampleTexture (void *indata, int inwidth, int inheight, void *outdata,  int outwidth, int outheight)
{
	...
	row1	= (byte *)malloc(outwidth * 4);
	row2	= (byte *)malloc(outwidth * 4);
...
GL_LoadTexture_setup:
	...
	if (!isDedicated)
	{
		...
		else if (bytesperpixel == 4)
		{
			if (mipmap)
			{
				GL_UploadMipmaps ((unsigned int *)data, width, height, alpha);
			}
			else
			{
				GL_Upload ((unsigned int *)data, width, height, alpha);
			}
		}
		else
		{
