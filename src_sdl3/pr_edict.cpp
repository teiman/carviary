...
Scans each function in progs.dat, computes a CRC hash of its bytecode,
and writes the results to carviary/data/analyze.json
===============
*/
void PR_Analyze_f (void)
{
	int			i, j;
	dfunction_t	*f;
	...
	if (!progs)
	{
		Con_Printf ("analyze: no progs loaded\n");
		return;
	}

	_snprintf (path, sizeof(path), "%s\\carviary\\data", host_parms.basedir);
	_mkdir (path);
	_snprintf (path, sizeof(path), "%s\\carviary\\data\\analyze.json", host_parms.basedir);

	fp = fopen (path, "w");
	if (!fp)
	{
		Con_Printf ("analyze: could not write %s\n", path);
		return;
	}
	...
	for (i = 1; i < progs->numfunctions; i++)
	{
		...
		const char *file = pr_strings + f->s_file;

		if (f->first_statement < 0)
		{
			// builtin function
			if (count > 0) fprintf (fp, ",\n");
			fprintf (fp, "  \"%s\": { \"builtin\": %d, \"file\": \"%s\" }",
				name, -f->first_statement, file);
			count++;
			continue;
		}

		// find the end of this function's statements:
		// it ends where the next function starts, or at numstatements
		int end_stmt;
		if (i + 1 < progs->numfunctions)
		{
			// find next non-builtin function
			end_stmt = progs->numstatements;
		...
		if (count > 0) fprintf (fp, ",\n");
		fprintf (fp, "  \"%s\": { \"hash\": \"%016llX\", \"statements\": %d, \"parms\": %d, \"locals\": %d, \"file\": \"%s\" }",
			name, hash, num_stmts, f->numparms, f->locals, file);
		count++;
	}
	...
	Con_Printf ("analyze: %d functions written to %s\n", count, path);
}
...
void PR_Init (void)
{
	...
	Cmd_AddCommand ("profile_qc", PR_Profile_f);
	Cmd_AddCommand ("analyze", PR_Analyze_f);
