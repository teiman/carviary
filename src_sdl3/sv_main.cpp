...
void SV_StartParticle (vec3_t org, vec3_t dir, int color, int count)
{
	...
	// Offline mode: run particle effect directly, skip network
	if (offline_mode)
	{
...
// find precache number for sound
	...
	// Offline mode: play sound directly, skip network serialization
	if (offline_mode)
	{
		vec3_t origin;
		for (i = 0; i < 3; i++)
...
// send an update
		...
		if (ent->baseline.modelindex != ent->v.modelindex)
			bits |= U_MODEL;
		if (sv.protocol == PROTOCOL_RMQ && ((int)ent->v.modelindex & 0xFF00))
			bits |= U_MODEL2;
...
void SV_WriteClientdataToMessage (edict_t *ent, sizebuf_t *msg)
{
	...
	// Offline mode: bypass stats/view serialization (damage/setangle go through net)
	if (offline_mode)
	{
		// Damage and setangle: let them serialize normally (rare, complex client parse)
		...
		// Everything else: direct copy, no serialization
		cl.viewheight = (int)ent->v.view_ofs[2];
		cl.idealpitch = (int)ent->v.idealpitch;
		VectorCopy(cl.mvelocity[0], cl.mvelocity[1]);
		for (i = 0; i < 3; i++)
		{
			cl.punchangle[i] = ent->v.punchangle[i];
			cl.mvelocity[0][i] = ent->v.velocity[i];
		}
		...
		cl.onground = ((int)ent->v.flags & FL_ONGROUND) != 0;
		cl.inwater = (ent->v.waterlevel >= 2);

		cl.stats[STAT_WEAPONFRAME] = (int)ent->v.weaponframe;
		cl.stats[STAT_ARMOR] = (int)ent->v.armorvalue;
		cl.stats[STAT_WEAPON] = SV_ModelIndex(pr_strings + ent->v.weaponmodel);
		cl.stats[STAT_HEALTH] = (int)ent->v.health;
		cl.stats[STAT_AMMO] = (int)ent->v.currentammo;
		cl.stats[STAT_SHELLS] = (int)ent->v.ammo_shells;
		cl.stats[STAT_NAILS] = (int)ent->v.ammo_nails;
		cl.stats[STAT_ROCKETS] = (int)ent->v.ammo_rockets;
		cl.stats[STAT_CELLS] = (int)ent->v.ammo_cells;
		cl.stats[STAT_ACTIVEWEAPON] = (int)ent->v.weapon;
...
// run two frames to allow everything to settle
	...
	// Auto-detect protocol: use 999 if precache counts exceed standard limits
	{
		int mc, sc;
		for (mc = 1; mc < MAX_MODELS && sv.model_precache[mc]; mc++);
		for (sc = 1; sc < MAX_SOUNDS && sv.sound_precache[sc]; sc++);
		if (mc > 255 || sc > 255)
		{
			sv.protocol = PROTOCOL_RMQ;
			sv.protocolflags = 0;
			Con_Printf("Using protocol 999 (%d models, %d sounds)\n", mc, sc);
		}
		else
		{
