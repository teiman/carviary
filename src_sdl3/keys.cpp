...
#include "sdlquake.h"	// Tomaz - Copy & Paste (SDL clipboard)
...
void Key_Console (int key)
{
	...
	// Tomaz - Copy & Paste (SDL3)
	if ((key=='V' || key=='v') && (SDL_GetModState() & SDL_KMOD_CTRL))
	{
		...
		if (clipText)
		{
			...
			if (i > 0)
			{
				char saved = clipText[i];
				clipText[i] = 0;
				strcat(key_lines[edit_line], clipText);
				key_linepos += i;
				clipText[i] = saved;
			}
			SDL_free(clipText);
	...
	// printable chars are handled by Key_CharEvent via WM_CHAR
	return;
}
...
Insert a character into the console command line
====================
*/
static void Key_InsertChar (int key)
{
...
Called from WM_CHAR with the OS-translated character.
Inserts into console or chat depending on key_dest.
====================
*/
