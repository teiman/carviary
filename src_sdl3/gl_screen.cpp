...
extern qboolean scr_drawdialog; // NOTE: only used in software mode??
...
void SCR_DrawFPS (void)
{
	...
	if (cl.time > fpscounter)
	{
		...
		_snprintf(temp_progs, sizeof(temp_progs),"qc:%.2fms", sv_progs_time);
		fpscounter = cl.time + 0.1;
	}

	Draw_String( vid.width - 64, 0, temp, 0);
	Draw_String( vid.width - 80, 8, temp_progs, 0);
}
