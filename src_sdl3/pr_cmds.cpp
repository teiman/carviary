...
void PF_normalize (void)
{
	...
	newlen = value1[0] * value1[0] + value1[1] * value1[1] + value1[2]*value1[2];
	newlen = sqrt(newlen);
	...
	{
		newlen = 1/newlen;
		newvalue[0] = value1[0] * newlen;
		newvalue[1] = value1[1] * newlen;
		newvalue[2] = value1[2] * newlen;
	}
...
void PF_precache_model (void)
{
	...
	for (i=0 ; i<MAX_MODELS ; i++)
	{
		if (!sv.model_precache[i])
		{
			...
			if (!sv.models[i])
				Con_Printf ("WARNING: model %s not found, skipping\n", s);
			return;
		}
...
void PF_makestatic (void)
{
	...
	if (sv.protocol == PROTOCOL_RMQ)
	{
		...
		if (midx & 0xFF00) bits |= B_LARGEMODEL;
		...
		if (bits)
		{
			...
			if (bits & B_LARGEMODEL) MSG_WriteShort(&sv.signon, midx);
			else MSG_WriteByte(&sv.signon, midx);
...
void PF_Fixme (void)
{
	// Non-fatal: extension builtins (e.g. AD's addstat #2101) are silently ignored
	Con_DPrintf ("PF_Fixme: unimplemented builtin called\n");
}
...
#define MAX_BUILTINS 4096
static builtin_t pr_builtin_extended[MAX_BUILTINS];
builtin_t *pr_builtins = pr_builtin_extended;
int pr_numbuiltins = MAX_BUILTINS;

void PR_InitBuiltins (void)
{
	int i;
	int base = sizeof(pr_builtin)/sizeof(pr_builtin[0]);
	for (i = 0; i < base; i++)
		pr_builtin_extended[i] = pr_builtin[i];
	for (; i < MAX_BUILTINS; i++)
		pr_builtin_extended[i] = PF_Fixme;
}
