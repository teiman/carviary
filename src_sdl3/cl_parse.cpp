...
// parse protocol version number
	i = MSG_ReadLong ();
	if (i != PROTOCOL_VERSION && i != PROTOCOL_RMQ)
	{
		Con_Printf ("Server returned version %i, not %i or %i\n", i, PROTOCOL_VERSION, PROTOCOL_RMQ);
		return;
	}
	cl.protocol = i;
	if (cl.protocol == PROTOCOL_RMQ)
		cl.protocolflags = MSG_ReadLong ();
	else
		cl.protocolflags = 0;
...
void CL_ParseBaseline (entity_t *ent, int version)
{
	...
	if (bits & B_LARGEFRAME)
		ent->baseline.frame = MSG_ReadShort ();
	else
